...
public class RouteAuthConfigurator implements RouteConfigurator {
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.route(RouteConstants.API_AUTH_ROUTE).handler(routingContext -> {
      ...
      String sessionToken =
          authorization != null ? authorization.substring(HttpConstants.TOKEN.length()).trim()
              : null;
      ...
      if (StringUtil.isNullOrEmpty(sessionToken)) {
        ...
        eBus.send(MessagebusEndpoints.MBEP_AUTH,
            new RouteRequestUtility().getBodyForMessage(routingContext), options, reply -> {
              if (reply.succeeded()) {
                ...
                if (responseHolder.isAnonymous()) {
                  ...
                  routingContext.request().params().add(MessageConstants.MSG_USER_ID,
                      responseHolder.getUserId());
                  routingContext.next();
                } else {
                  routingContext.response()
